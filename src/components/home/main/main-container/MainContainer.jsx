  ...
  useEffect(() => {
    const forceWebGLSettings = () => {
      ...
      for (const context of contexts) {
        try {
          ...
          console.warn(`Contexto ${context} falló:`, e);
        }
      }

      if (!gl) {
        setErrorInfo("WebGL completamente no disponible en este dispositivo");
        setShowError(true);
        return false;
      }
      ...
      try {
        ...
        extensions.forEach((ext) => {
          ...
          if (extension) {
            console.log(`✅ Extensión forzada: ${ext}`);
          }
        });
      } catch (e) {
        console.warn("Error forzando extensiones:", e);
      }
    ...
    const forceMemoryOptimization = () => {
      ...
      if (navigator.userAgent.includes("Chrome")) {
        performance.mark("spline-load-start");
      }
    };
